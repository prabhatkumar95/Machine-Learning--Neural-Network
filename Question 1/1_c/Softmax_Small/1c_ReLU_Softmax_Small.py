...
def data_load():
    file = h5py.File('/home/iiitd/Music/HW-3-NN/dataset_partA.h5', 'r')
...
def plot_graph(epoch, accu, count):
    ...
    pyplot.title('Question 1c_relu_softmax_small_'+str(count))
    pyplot.plot(epoch, accu)
    pyplot.savefig('1c_relu_softmax_small_'+str(count)+'.jpg', dpi=100)
...
f = open('1c_relu_softmax_small_Accuracy.txt', 'w')
a = [784, 100, 50, 2]
objects = []
pos = -1
max_learn = 0.0
...
f.close()
with open('1c_relu_softmax_small.pkl', 'wb') as output:
